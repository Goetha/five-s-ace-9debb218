    ...
    try {
      ...
      const formattedAudits = await Promise.all(
        (auditsData || []).map(async (audit) => {
          const [companyRes, locationRes, profileRes] = await Promise.all([
            supabase.from('companies').select('name').eq('id', audit.company_id).single(),
            supabase.from('environments').select('name').eq('id', audit.location_id).single(),
            supabase.from('profiles').select('full_name').eq('id', audit.auditor_id).single()
          ]);
      ...
      const scheduled = formattedAudits.filter(
        audit => audit.status === 'completed' && 
                 audit.next_audit_date && 
                 new Date(audit.next_audit_date) > new Date()
      );
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {scheduledAudits.map(audit => (
                <div key={audit.id} className="flex items-center justify-between p-3 bg-purple-50 border border-purple-200 rounded-lg">
                  <div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                <p className="text-center text-muted-foreground py-8">Nenhuma auditoria encontrada</p>
              ) : (
