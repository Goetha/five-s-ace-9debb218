      ...
      {criteria.map(criterion => <Card key={criterion.id} className={`hover:shadow-lg transition-all relative overflow-hidden ${getSensoBackgroundColor(criterion.senso)}`}>
          <CardContent className="p-6 space-y-4 relative">
            ...
            <div className="flex flex-wrap gap-1.5">
              {criterion.tags.map(tag => <Badge key={tag} variant="secondary" className="text-xs bg-background/60">
                  {tag}
                </Badge>)}
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-border/50">
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-background/80" onClick={() => onEdit(criterion)}>
                  <Edit className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-background/80" onClick={() => onDuplicate(criterion)}>
                  <Copy className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-background/80" onClick={() => onView(criterion)}>
